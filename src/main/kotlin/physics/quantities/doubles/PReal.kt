...
class PReal internal constructor(
    ...
    private fun Double.suppressDwindlingDigits(): Double {
        ...
        if (splitOnE.size > 1) {
            ...
            asString = if (exponent < 0) {
                (if (asString.first() == '-') "-" else "") + ("0." + "0".repeat(-exponent - 1) + coefficient.remove("-").remove(".")).removeSuffix("0")
            } else {
    ...
    fun pow(other: PReal): PReal {
        ...
        return PReal(
            ...
            min(significantDigitsCount, other.significantDigitsCount),
            unit.pow(other.valueStorage.toDouble())
        )
    }
    ...
    fun convertInto(unit: PUnit): PReal {
        ...
        return PReal(
            this.unit.convert(valueStorage, into = unit) ?: throw UnitException("Unit ${this.unit} cannot be converted into $unit."),
            significantDigitsCount,
            unit
        )
    }
    ...
    fun isFinite(): Boolean = !isPositiveInfinity() && !isNegativeInfinity()
}
