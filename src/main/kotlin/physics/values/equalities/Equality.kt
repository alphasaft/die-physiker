...
class Equality(
    ...
    fun isolateVariable(variable: String): Equality = when (val variableAsExpression = Var(variable)) {
        this.left -> this
        in this.left -> Equality(variableAsExpression, left.isolateVariable(variable).invoke(this.right))
        in this.right -> Equality(variableAsExpression, right.isolateVariable(variable).invoke(this.left))
    ...
    fun compute(variable: String, arguments: Map<String, Quantity<PReal>>): Quantity<PReal> {
        return isolateVariable(variable).right.evaluate(arguments)
    }
