...
class Sum(terms: List<Expression>): Expression() {
    ...
    private fun factoriseWithRemarquableIdentities(members: List<Expression>): List<Expression> {
        ...
        main@ for (a in filtered) {
            for (b in filtered - a) {
                ...
                result = applyIdentityTo(result, a.x, b.x, (a.exponent as Const).value.asPValue().convertTo<PReal>().toInt())
                if (previous != result) continue@main
            }

            if (constant == null) continue
            val rootOfConst = constant.pow(Const(1) /a.exponent)
            result = applyIdentityTo(result, a.x, rootOfConst, (a.exponent as Const).value.asPValue().convertTo<PReal>().toInt())
        }
    ...
    override fun evaluate(arguments: Map<String, Quantity<PReal>>): Quantity<PReal> {
        return members.map { it.evaluate(arguments) }.reduce { a, b -> a + b }
    }
